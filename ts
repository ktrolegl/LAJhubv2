...
if game:GetService("UserInputService").TouchEnabled and not table.find({"Solara","Wave","Wind","Synapse Z"},executor) then codex, fluxus, delta, arceus=true,true,true,true end
...
if BF == true then
    ...
    -- Webhook and Key system removed
    webhook = {
        ["item_webhook"] = os.getenv("DISCORD_WEBHOOK_URL"),
        ["level_webhook"] = os.getenv("DISCORD_WEBHOOK_URL"),
        ["misc_webhook"] = os.getenv("DISCORD_WEBHOOK_URL"),
    }
    
    -- Webhook function using Discord Webhook URL from environment variable
    function sendWebhook(url, content, embed)
        if url then
            ...
            if not success then
                print("LAJ HUB | Webhook Error: " .. tostring(response))
            end
        end
    end

    if _G.LAJHUBLoaded == 1 then
        Notification("Another instance of LAJ HUB already loaded")
        print("Another instance of LAJ HUB already loaded")
        return
    end

    local SettingsFile = "LAJHUBUI.json"
    ...
    function WriteSettings()
        writefile(SettingsFile, game:GetService('HttpService'):JSONEncode(getgenv().LAJHUBSettings))
    end
    ...
    getgenv().LAJHUBSettings = DefaultSettings
    -- For backwards compatibility
    getgenv().EuphoriaSettings = getgenv().LAJHUBSettings
    
    settings = LoadSettings()
    if settings then
        for i,v in pairs(settings) do
            getgenv().LAJHUBSettings[i] = v
        end 
    end
    WriteSettings()
    ...
    -- Webhook functions for different item types
    function sendWeaponWebhook(newWeapons, webhook)
        if webhook then
            local embed = {
                {
                    ["title"] = "LAJ HUB | New Weapons",
                    ["description"] = "**New items obtained:**\n" .. table.concat(newWeapons, "\n"),
                    ["color"] = 0x00FF00,
                    ["footer"] = {
                        ["text"] = "LAJ HUB | " .. os.date("%Y-%m-%d %H:%M:%S")
                    }
                }
            }
    ...
    function sendAccessoryWebhook(newAccs, webhook)
        if webhook then
            local embed = {
                {
                    ["title"] = "LAJ HUB | New Accessories",
                    ["description"] = "**New accessories obtained:**\n" .. table.concat(newAccs, "\n"),
                    ["color"] = 0xFF00FF,
                    ["footer"] = {
                        ["text"] = "LAJ HUB | " .. os.date("%Y-%m-%d %H:%M:%S")
                    }
    ...
    function sendMaterialWebhook(newMaterial, webhook)
        if webhook then
            local embed = {
                {
                    ["title"] = "LAJ HUB | New Materials",
                    ["description"] = "**New materials obtained:**\n" .. table.concat(newMaterial, "\n"),
                    ["color"] = 0x0000FF,
                    ["footer"] = {
                        ["text"] = "LAJ HUB | " .. os.date("%Y-%m-%d %H:%M:%S")
                    }
    ...
    notis.new("<Color=Red>LAJ HUB<Color=/><Color=Yellow> loaded successfully<Color=/>"):Display()
    notis.new("<Color=Red>LAJ HUB<Color=/><Color=Yellow> loaded successfully<Color=/>"):Display()
    notis.new("<Color=Red>LAJ HUB<Color=/><Color=Yellow> loaded successfully<Color=/>"):Display()
    ...
    local Library = loadstring(game:HttpGet("https://you.whimper.xyz/sources/EuphoriaUi"))()
    local Window = Library.CreateLib("25ms was here", "DarkTheme")
            ...
            while wait(2) do
                pcall(function()
                    if PlayerData.BlueGear then
                        DialogueStatus:UpdateLabel("Blue Gear: âœ…")
                        return
                    end
                    if GetMaterial("Mirror Fractal") <= 0 or not isAccessoryInInventory("Valkyrie Helm") then
                        ...
                        if GetMaterial("Mirror Fractal") <= 0 and not isAccessoryInInventory("Valkyrie Helm") then
                            tex = tex .. " Mirror Fractal and Valkyrie Helm âŒ"
                        elseif not isAccessoryInInventory("Valkyrie Helm") then
                            tex = tex .. " Valkyrie Helm âŒ"
                        elseif GetMaterial("Mirror Fractal") <= 0 then
                            tex = tex .. " Mirror Fractal âŒ"
                        end
                        DialogueStatus:UpdateLabel("Blue Gear: " .. tex)
                        return
                    end 
                    if PlayerData.RaceVer ~= "V3" then
                        DialogueStatus:UpdateLabel("Blue Gear: Need to get V3 Race âŒ")
                        return
                    end
                    if PlayerData.V4Status == 1 then
                        DialogueStatus:UpdateLabel("Blue Gear: Need complete dialog (1/4) âŒ")
                    elseif PlayerData.V4Status == 2 then
                        DialogueStatus:UpdateLabel("Blue Gear: Need complete dialog (2/4) âŒ")
                    elseif PlayerData.V4Status == 3 then
                        DialogueStatus:UpdateLabel("Blue Gear: Need complete dialog (3/4) âŒ")
                    elseif PlayerData.V4Status == 4 then
                        DialogueStatus:UpdateLabel("Blue Gear: Need to get Blue Gear âŒ")
                    end
                end)
            ...
            while wait() do
                pcall(function()
                    if game:GetService("Workspace").Map:FindFirstChild("MysticIsland") then
                        MirageIslandDistance:UpdateLabel("Mirage Spawned: âœ… | " .. tostring(math.floor(getDistance(CFrame.new(game:GetService("Workspace").Map.MysticIsland.Center.Position.X,500,game:GetService("Workspace").Map.MysticIsland.Center.Position.Z)))) .. "m (" .. tostring(math.floor(getDistance(CFrame.new(game:GetService("Workspace").Map.MysticIsland.Center.Position.X,500,game:GetService("Workspace").Map.MysticIsland.Center.Position.Z)) / _G.TweenSpeed)) .. " +- seconds)")
                    else
                        MirageIslandDistance:UpdateLabel("Mirage Spawned: âŒ")
                    end
                end)
            end
        end)
            ...
            while wait() do
                pcall(function()
                    if BribeLevel == -1 then
                        BribeLabel:UpdateLabel("Bribe: Complete 20 sea events âŒ")
                    else
