...
Fin:CreateButton({
   ...
   Callback = function()
        ...
        local success, errorMsg = pcall(function()
            ...
            local scriptUrl = "https://raw.githubusercontent.com/Nivex123456/main/refs/heads/main/Loader.lua"
            
            -- Try to get the script content
            local scriptContent = game:HttpGet(scriptUrl)
            
            -- If we got content, run it with a protected call
            if scriptContent and scriptContent ~= "" then
                -- Set any possible bypass variables
                if getgenv then
                    getgenv().KeySystem = false
                    getgenv().BypassSystem = true
                end
                
                -- Execute the script
                loadstring(scriptContent)()
            else
                error("Failed to get script content")
            end
        end)
        ...
        if success then
            ...
            warn("LAJ HUB: Failed to load Fisch Nivex - " .. tostring(errorMsg))
            
            -- Try an alternative loading method
            Rayfield:Notify({
                Title = "Fisch Nivex",
                Content = "Trying alternative loading method...",
                Duration = 3,
                Image = 4483362458,
            })
            
            -- Try direct loading without checking content
            pcall(function()
                loadstring(game:HttpGet("https://raw.githubusercontent.com/Nivex123456/main/refs/heads/main/Loader.lua", true))()
            end)
        end
   end,
})
